...
public class ShowInterestTask extends AsyncTask<String, Void, String> {
	...
	public ShowInterestTask(InterestFragment interestFragment) {
		super();
	...
	protected void onPostExecute(String result) {
		...
		if (interests != null) { // 兴趣列表获取成功
			...
			for(Interests interest:interests) {
				interestFragment.dataInterest.add(interest);
			}
			System.out.println("notify");
			interestFragment.adapterInterest.notifyDataSetChanged();
		} else {
			System.out.println("兴趣标签加载错误");
		}
		Log.d("INFO2", interestFragment.dataInterest.get(0).getInt_name());
	}
}
